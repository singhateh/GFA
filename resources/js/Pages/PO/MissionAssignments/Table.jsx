...
const MedicalClearanceTable = ({ clearances, onEdit, onDelete }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 5;

    // Pagination logic
    const totalPages = Math.ceil(clearances.length / itemsPerPage);
    const paginatedData = clearances.slice(
                ...
                <table className="min-w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-md">
                    <thead>
                        <tr className="bg-gray-100 dark:bg-gray-700 text-left text-gray-900 dark:text-gray-100">
                            {["Staff Member", "Issued Date", "Type", "Status", "Remarks", "Actions"].map((header, index) => (
                                <th key={index} className="py-2 px-4">{header}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {paginatedData.map((clearance) => (
                            <tr key={clearance.id} className="border-t border-gray-200 dark:border-gray-700">
                                <td className="py-2 px-4 text-gray-900 dark:text-gray-100">
                                    {truncateText(clearance.staff_member?.name)}
                                </td>
                                <td className="py-2 px-4 text-gray-900 dark:text-gray-100">
                                    {clearance.issued_at}
                                </td>
                                <td className="py-2 px-4 text-gray-900 dark:text-gray-100">
                                    {truncateText(clearance.type)}
                                </td>
                                <td className="py-2 px-4">{renderStatusBadge(clearance.status)}</td>
                                <td className="py-2 px-4 text-gray-900 dark:text-gray-100">
                                    {truncateText(clearance.remarks)}
                                </td>
                                <td className="py-3 px-4">
                                    <button onClick={() => onEdit(clearance)}
                                        className="px-3 py-1 bg-yellow-500 text-white rounded mr-2 hover:bg-yellow-600 transition">
                                        Edit
                                    </button>
                                    <button onClick={() => onDelete(clearance.id)}
...
export default MedicalClearanceTable;
