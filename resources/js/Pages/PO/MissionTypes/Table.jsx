...
const MissionTypeTable = ({ missionTypes, onEdit, onDelete }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 5;

    // Pagination logic
    const totalPages = Math.ceil(missionTypes.length / itemsPerPage);
    const paginatedData = missionTypes.slice(
                ...
                <table className="min-w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-md">
                    <thead>
                        <tr className="bg-gray-100 dark:bg-gray-700 text-left text-gray-900 dark:text-gray-100">
                            {["Name", "Description", "Actions"].map((header, index) => (
                                <th key={index} className="py-2 px-4">{header}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {paginatedData.map((missionType) => (
                            <tr key={missionType.id} className="border-t border-gray-200 dark:border-gray-700">
                                <td className="py-2 px-4 text-gray-900 dark:text-gray-100">
                                    {truncateText(missionType?.name)}
                                </td>

                                <td className="py-2 px-4 text-gray-900 dark:text-gray-100">
                                    {truncateText(missionType.description)}
                                </td>

                                <td className="py-3 px-4">
                                    <button onClick={() => onEdit(missionType)}
                                        className="px-3 py-1 bg-yellow-500 text-white rounded mr-2 hover:bg-yellow-600 transition">
                                        Edit
                                    </button>
                                    <button onClick={() => onDelete(missionType.id)}
...
export default MissionTypeTable;
